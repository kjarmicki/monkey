...
// Pratt's parser main idea: association of parsing functions with token types (prefix / infix)
type (
	prefixParseFn func() ast.Expression               // called when token is encountered in prefix position
	infixParseFn  func(ast.Expression) ast.Expression // called when token is encountered in infix position
)
...
// this map defines which tokens have the same precedence
var predences = map[token.TokenType]int{
	token.EQ:       EQUALS,
	token.NOT_EQ:   EQUALS,
...
func (p *Parser) parseExpression(precedence int) ast.Expression {
	...
	// once we have the left expression (prefix), try to match the right one (infix)
	// loop until the next semicolon or bigger precedence
	// warning: leftExp is not just left expression anymore! it's everything we were able to match
...
func (p *Parser) peekPrecedence() int {
	if p, ok := predences[p.peekToken.Type]; ok {
		return p
	}
	return LOWEST
}
...
func (p *Parser) curPrecedence() int {
	if p, ok := predences[p.curToken.Type]; ok {
		return p
	}
	return LOWEST
}
